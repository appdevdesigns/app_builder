...
module.exports = {
   ...
   sendFeeConfirmation: function(req, res) {
      ...
      var hashFeesChildcare = {
         /* fee.id : {fee} */
      };
      var hashFeesSchedule = {
         /* fee.id : {fee} */
      };
      var hashFeesMisc = {
         /* fee.id : {fee} */
         ...
         [
            ...
            (next) => {
               ...
               async.each(
                  ...
                  (regID, cb) => {
                     ...
                     var allRegistrantIDs = Object.keys(
                        hashPackets[regID].registrants
                     );
                     async.each(
                        allRegistrantIDs,
                        (registrantID, done) => {
                           var registrant =
                              hashPackets[regID].registrants[registrantID];
                        ...
                        (err) => {
                              ...
                              if (packet.registrants[rID].Attending == 1) {
                                 if (packet.registrants[rID].rendata) {
                                    var name = getFullName(
                                       packet.registrants[rID].rendata
                                    );
                                    packet.attendees.push(name);
                                 } else {
                                    packet.error = true;
                                    packet.errorText.push(
                                       "No Ren for Registrant:" + rID
                                    );
                                    errors.push(rID);
                                 }
                              }
                           ...
                           if (errors.length) {
                              resultErrorMissingRen.push({
                                 packet: packet,
                                 registrantIDs: errors
                              });
                           }
                        ...
                        (err, results, fields) => {
                           if (err) cb(err);
                              ...
                              results.forEach((r) => {
                                 ...
                                 var foundRen = Object.keys(packet.registrants)
                                    .map((rKey) => {
                                       return packet.registrants[rKey];
                                    })
                                    .find((reg) => {
                                       return reg["Ren Name"] == r.Ren;
                                    });
                                 if (foundRen) {
                                    packet.preferredName = getFirstName(
                                       foundRen.rendata
                                    );
                                 } else {
                                    var firstID = Object.keys(
                                       packet.registrants
                                    )[0];
                                    if (firstID) {
                                       if (
                                          packet.registrants[firstID].rendata
                                       ) {
                                          packet.preferredName = getFirstName(
                                             packet.registrants[firstID].rendata
                                          );
                                       ...
                                       packet.error = true;
                                       packet.errorText.push(
                                          "No Registrants for this Registration"
                                       );
                                    }
                                 }
                              });
            ...
            (next) => {
               ...
               allRegistrationIDs.forEach((registrationID) => {
                  ...
                  var langTrans = json.find((t) => {
                     return t.language_code == packet.languageCode;
                  });
                  if (langTrans) {
                     ...
                     if (packet.event) {
                        ...
                        packet.error = true;
                        packet.errorText.push(
                           "No event for this registration:" +
                              packet.registration.id
                        );
                     }
                  }
               });
...
`;
               ...
               var stringResultSentEmails = JSON.stringify(
                  formatResultSentEmails,
                  null,
                  4
               );
               logContents += `
...
`;
               ...
               console.log(
                  "... there were " +
                     resultErrorPackets.length +
                     " registrations with errors compiling their data...",
                  resultErrorPackets
               );
               ...
               resultErrorPackets.forEach((packet) => {
                  formatCompileErrors[
                     packet.registration.id
                  ] = packet.errorText.join("; ");
               });
               var stringCompileErrors = JSON.stringify(
                  formatCompileErrors,
                  null,
                  4
               );
...
`;
               ...
               resultErrorMissingRen.forEach((entry) => {
                  formatErrorMissingRen[entry.packet.registration.id] =
                     entry.registrantIDs;
               });
               var stringErrorMissingRen = JSON.stringify(
                  formatErrorMissingRen,
                  null,
                  4
               );
...
`;
               ...
               fs.writeFile(
                  "events_log_feeEmails_" + tsFlag + ".log",
                  logContents,
                  (err) => {
                     if (err) {
                        ADCore.error.log(
                           "::: ABMobileQRController.sendFeeConfirmation(): error writing log file: ",
                           { error: err, logContents: logContents }
                        );
                     }
                     next();
                  }
            ...
            (next) => {
               ...
               function populateRegistrantRen(list, cb) {
                  if (list.length == 0) {
                     ...
                     var allRegistrantIDs = Object.keys(
                        hashRegistrationPackets[registrationID].registrants
                     );
                     populateRen(
                        allRegistrantIDs,
                        hashRegistrationPackets[registrationID].registrants,
            ...
            (next) => {
               ...
               function eachRegistration(list, cb) {
                  if (list.length == 0) {
                     ...
                     var attendees = [];
                     if (!packet.error) {
                        Object.keys(packet.registrants)
                           .map((r) => {
                              return packet.registrants[r];
                           })
                           .forEach((person) => {
                              if (person.Attending == 1) {
                                 if (person.rendata) {
                                    attendees.push({
                                       ...
                                       photo:
                                          hashPhotos[person.rendata.ren_id] &&
                                          hashPhotos[person.rendata.ren_id] !=
                                             ""
                                    });
                                 } else {
                           ...
                           if (err) cb(err);
                              ...
                              results.forEach((r) => {
                                 ...
                                 if (hashHousingRoomFees[r.Fees177]) {
                                    var from = moment(r.Start).format(
                                       "ddd MMM D"
                                    );
            ...
            (next) => {
               ...
               function eachRegistration(list, cb) {
                  if (list.length == 0) {
                     ...
                     var allRegistrants = Object.keys(packet.registrants).map(
                        (k) => {
                           return packet.registrants[k];
                        }
                     );
                     var childRenType = 4;
                     var lang = packet.languageCode || "en";
                        ...
                        if (registrant.Attending) {
                           ...
                           if (ren && ren.rentype_id == childRenType) {
                              childCare.push({
                                 name: getFirstName(ren),
                                 option:
                                    hashChildCare[registrant.Childcare][lang]
                              });
                           }
                        }
                     });
                        ...
                        if (registrant.Attending) {
                           ...
                           if (ren && ren.rentype_id != childRenType) {
                              translations.push({
                                 name: getFirstName(ren),
                                 option:
                                    hashTranslation[registrant.translation][
                                       lang
                                    ]
                              });
                           }
...
`;
               ...
               console.log(
                  "... there were " +
                     resultErrorSending.length +
                     " registrations with errors sending their emails."
               );
               ...
               var stringErrorSending = JSON.stringify(
                  formatErrorSending,
                  null,
                  4
               );
...
`;
               ...
               fs.writeFile(
                  "events_log_confirmationEmailErrors_" + tsFlag + ".log",
                  logContents,
                  (err) => {
                     if (err) {
                        ADCore.error.log(
                           "::: ABMobileQRController.sendRegistrationConfirmation(): error writing log file: ",
                           { error: err }
                        );
                     }
                     next();
   ...
   receiveRegistrationConfirmationResponse: function(req, res) {
      ...
      console.log(
         "::: Event Registration Confirmed: " + regID + " " + isConfirmed
      );
