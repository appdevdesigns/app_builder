...
module.exports = {
      ...
      init: function() {
         ...
         bd(function(err) {
            ...
            if (err) {
               ...
               ADCore.error.log(
                  "AppBuilder:buildDirectory.init(): exited with an error",
                  {
                     error: err,
               ...
               return new Promise((next, err) => {
                  ...
                  ABGraphObject.findOne(objID)
                     ...
                     .then(function(object) {
                        if (object) result = object.toABClass();
            ...
            .then(() => {
               return new Promise((next, err) => {
                  ...
                  ABGraphQuery.findOne(objID, {
                     ...
                     .catch(err)
                     .then(function(query) {
                        if (query) result = query.toABClass();
                  ...
                  ABGraphApplication.findOne(appID)
                     .then(function(app) {
                        if (!app) {
                           ...
                           ADCore.error.log("Application not found ", {
                              error: err,
                              appID: appID
                           });
                           next(err);
   ...
   buildObject: function(objectID) {
      ...
      async.series(
         [
               ...
               if (importFromObjectId) {
                  ...
                  ABObject.find({ id: importFromObjectId })
                     ...
                     .then(function(list) {
                        ...
                        if (!obj)
                           throw new Error(
                              "invalid import object id: " + importFromObjectId
                           );
                        else next();
                     })
                     .catch(function(err) {
            ...
            function(next) {
               // Only numbers and alphabets will be used
               appName = AppBuilder.rules.toApplicationNameFormat(
            ...
            function(next) {
               ...
               async.each(
                  [fullPath, fullPathTrans, clientPath, baseClientPath],
                  function(target, ok) {
                     // Delete file if it exists
                     fs.unlink(target, function(err) {
                        // Ignore errors. If file does not exist, that's fine.
                     ...
                     function(callback) {
                        async.eachSeries(
                           ...
                           function(col, ok) {
                              ...
                              if (!field) {
                                 ok(
                                    "System could not found this field type: " +
                                       col.fieldName
                                 );
                                 return;
                              }
            ...
            function(next) {
               ...
               var pathModelDir = path.join(
                  AppBuilder.paths.sailsBuildDir(),
                  "api",
                  "models"
               );
               ...
               function importModel(file) {
                  ...
                  hashCurrModels[modelRef] = require(path.join(
                     pathModelDir,
                     file
                  ));
               }
               ...
               function calcOffsetPath(fileName, currPath) {
                  ////  calculate the relative offset for our original (live) model file:
                  var offsetDir = currPath.replace(
                     sails.config.appPath + path.sep,
                     ""
                  );
               ...
               function linkModel(name, cb) {
                  ...
                  var livePath = calcOffsetPath(
                     path.join(modelsPath, name) + ".js",
                     destPath
                  );
                  // console.log('... livePath:'+livePath);
                  // console.log('... cwd:', process.cwd() );

                  var currLivePath = calcOffsetPath(
                     path.join(modelsPath, name) + ".js",
                     process.cwd()
                  );
            ...
            async.series(
               [
                  ...
                  function(next) {
                     function pullData() {
                        appName = AppBuilder.rules.toApplicationNameFormat(
                           applications[page.application].name
                        );
                        pageName = AppBuilder.rules.nameFilter(page.name);
                     ...
                     if (applications[page.application]) {
                        ...
                        ABGraphApplication.findOne(page.application).then(
                           function(result) {
                              if (result) {
                  ...
                  function(next) {
                     OPConfigTool.destroy({ key: pageKey }).then(function(
                        result
                     ) {
                        next();
                     },
                  ...
                  // Remove a Tool Definition for the OP Portal Navigation
                  function(next) {
                     OPSPortal.NavBar.ToolDefinition.remove(pageKey).then(
                        function() {
                           next();
                        },
                        next
                  ...
                  function ABPage_AfterDelete_RemovePermissions(next) {
                     Permissions.action
                        .destroyKeys([page.permissionActionKey])
                        .then(function(data) {
                           next();
                        }, next);
                  }
   ...
   exportPage: function(pageID) {
      ...
      async.series(
         [
                     ...
                     for (var i = 0; i < list.length; i++) {
                        ...
                        objectIncludes.push({
                           key:
                              ...
                              appName +
                              "_" +
                              AppBuilder.rules.nameFilter(obj.name),
                           path:
                              "opstools/" +
                              appName +
                              "/models/" +
            ...
            function(next) {
               sails.renderView(
                  path.join("app_builder", "page_controller"),
                  {
                     layout: false,
                     appId: appID,
                     appName: appName,
   ...
   findModelAttributes: function(modelName) {
      ...
      var transAssoc = model.associations.filter(function(assoc) {
         return assoc.alias == "translations" && assoc.type == "collection";
      })[0];
      if (transAssoc) {
