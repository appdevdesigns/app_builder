...
module.exports = function(cb) {
   ...
   async.series(
      [
         ...
         function(next) {
            var parts = sailsBuildPath
               .replace(sails.config.appPath + path.sep, "")
               .split(path.sep);
            sailsDir = parts.pop();
            ...
            createPath(parts, sails.config.appPath, function(err) {
               if (err) {
                  ADCore.error.log(
                     "AppBuilder:buildDirectory.init:build_directory.js Unable to create our sails build directory",
                     err
                  );
                  next(err);
               } else {
      ...
      [
         ...
         function(next) {
            ...
            fs.unlink(pathDestConfig, function(err) {
               ...
               fs.writeFile(
                  pathDestConfig,
                  "module.exports = " + JSON.stringify(destConfig, null, 4),
                  function(err) {
                     next(err);
                  }
               );
         ...
         function(next) {
            ...
            var pathHook = destSails(
               path.join("api", "hooks", "exitAfterORM.js")
            );
