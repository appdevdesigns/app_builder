   ...
   if (Array.isArray(value)) {
      for (var i = 0; i < value.length; i++) {
         ...
         if (newID) {
            ...
            throw new Error(
               `Unable to remap ${settingKey} ${objType}: ${oldID}`
            );
         }
      }
   } else {
...
var uniqueName = function(name, type, langCode) {
   ...
   async.during(
      ...
      function(done) {
         ...
         model.find(cond).exec(function(err, list) {
            if (err) {
               sails.log(err);
               // Error. Abort.
               done(err);
      ...
      async.series(
         [
            ...
            function(next) {
               ...
               ABApplication.create(appData).exec(function(err, result) {
                  if (err) next(err);
                  else {
                     appID = result.id;
                     next();
                  ...
                  function(trans, transDone) {
                     uniqueName(trans.label, "label", trans.language_code)
                        ...
                        .done(function(uniqueResult) {
                           ...
                           ABApplicationTrans.create(transData).exec(function(
                              err,
                              result
                           ) {
            ...
            function(next) {
               async.each(
                  ...
                  function(obj, objDone) {
                     ...
                     ABObject.create(objData).exec(function(err, result) {
                        if (err) objDone(err);
                        else {
                           var newID = result.id;
                           ...
                           async.each(
                              ...
                              function(trans, transDone) {
                                 ...
                                 ABObjectTrans.create(transData).exec(function(
                                    err,
                                    result
                                 ) {
                                    if (err) transDone(err);
            ...
            function(next) {
               async.each(
                  ...
                  function(col, colDone) {
                     ...
                     ABColumn.create(colData).exec(function(err, result) {
                        if (err) colDone(err);
                        else {
                           ...
                           async.each(
                              ...
                              function(trans, transDone) {
                                 ...
                                 ABColumnTrans.create(transData).exec(function(
                                    err,
                                    result
                                 ) {
                                    if (err) transDone(err);
            ...
            function(next) {
               async.each(
                  columnsNeedRemap,
                  function(colID, colDone) {
                     ABColumn.findOne({ id: colID }).exec(function(
                        err,
                        result
                     ) {
                        if (err) colDone(err);
            ...
            function(next) {
               async.each(
                  data.lists,
                  function(list, listDone) {
                     var listData = {
                        column: reference.columns.map[list.column],
                        key: list.key,
                        value: list.value
                     };
                     ABList.create(listData).exec(function(err, result) {
                        if (err) listDone(err);
                        else {
                           ...
                           async.each(
                              ...
                              function(trans, transDone) {
                                 ...
                                 ABListTrans.create(transData).exec(function(
                                    err
                                 ) {
                                    if (err) transDone(err);
                                    else transDone();
            ...
            function(next) {
               async.each(
                  ...
                  function(page, pageDone) {
                     ...
                     ABPage.create(pageData).exec(function(err, result) {
                        if (err) pageDone(err);
                        else {
                           ...
                           async.each(
                              ...
                              function(trans, transDone) {
                                 ...
                                 ABPageTrans.create(transData).exec(function(
                                    err,
                                    result
                                 ) {
                                    if (err) transDone(err);
            ...
            function(next) {
               async.eachOf(
                  reference.pages.map,
                  function(pageID, oldPageID, pageDone) {
                     ABPage.findOne({ id: pageID }).exec(function(err, result) {
                        if (err) pageDone(err);
                        else if (!result) {
                           ...
                           result.parent =
                              reference.pages.map[parseInt(result.parent)];
                           ABPage.update(
                              { id: pageID },
                  ...
                  function(comp, compDone) {
                     ...
                     if (comp.setting) {
                        try {
                           ...
                           ["pageIds", "pages", "viewPage", "editPage"].forEach(
                              function(key) {
                                 remap(comp, key, "pages", reference);
                              }
                           );
                     ...
                     ABPageComponent.create(compData).exec(function(
                        err,
                        result
                     ) {
                        if (err) compDone(err);
            ...
            function(next) {
               async.each(
                  componentsNeedRemap,
                  function(compID, compDone) {
                     ABPageComponent.findOne({ id: compID }).exec(function(
                        err,
                        result
                     ) {
                        if (err) compDone(err);
                           ...
                           compDone(
                              new Error("component not found: " + compID)
                           );
                        } else {
                           try {
                              ["viewId", "editForm"].forEach(function(key) {
