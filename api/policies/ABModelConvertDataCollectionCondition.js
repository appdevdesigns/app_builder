...
function findDcEntry(_where) {
    ...
    if (_where.rules) {
        ...
        if ((_where.rule == 'in_data_collection') || (_where.rule == 'not_in_data_collection')) {
            return _where;
        } else {
            return null;
        }
    ...
    if (!cond) {
        ...
        if (!object) {
            AppBuilder.routes.verifyAndReturnObject(req, res)
                ...
                .catch((err)=>{
                    ADCore.error.log('AppBuilder:Policy:ABModelConvertDataCollectionCondition:error finding object for this route:', {error:err});
            ...
            if (!dc) {
                ...
                var err = new Error('Unknown Data collection ID in condition.');
                err.condition = cond;
                cb(err);
                return;
            ...
            if (cond.key == 'this_object') {
                ...
                objectColumn = (cond.alias ? cond.alias : object.dbTableName(true))+'.'+object.PK();
                ...
                if (!field) {
                    ...
                    if (!field) {
                        ...
                        ADCore.error.log('AppBuilder:Policy:ABModelConvertDataCollectionCondition:Unable to resolve condition field.:', { field:cond.key, condition:cond });                    
                ...
                var linkCase = field.linkType()+':'+field.linkViaType();
                    ...
                    case 'one:many':
                        ...
                        objectColumn = sourceObject.dbTableName(true)+'.'+parseColumn;   
                        continueSingle(newKey, parseColumn, objectColumn);                             
                        break;
                    ...
                    case 'many:many':
                        ...
                        processQueryValues(parseColumn,  objectColumn,  (err, ids) => {
            ...
            function processQueryValues(parseColumn,  objectColumn,  done) {
                ...
                query
                    ...
                    .catch((err)=>{
                        ADCore.error.log('AppBuilder:Policy:ABModelConvertDataCollectionCondition:Error running query:', { error:err });
                        done(err);
                    })
            }
