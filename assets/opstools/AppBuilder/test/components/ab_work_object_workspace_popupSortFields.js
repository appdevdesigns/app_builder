...
describe("ab_work_object_workspace_popupSortFields component", () => {
   ...
   describe("Logic testing", () => {
      ...
      it(".clickAddNewSort: should add a new element to UI", () => {
         ...
         let addNewSortButtonClickFn =
               target.ui.body.elements[target.ui.body.elements.length - 1].on
                  .onItemClick,
            onShowFn = target.ui.on.onShow,
            spyLogicOnChange = sandbox.spy(target._logic.callbacks, "onChange"),
            spyLogicClickAddNewSort = sandbox.spy(
               target._logic,
               "clickAddNewSort"
            );
         ...
         sandbox.assert.callCount(
            spyLogicClickAddNewSort,
            mockObj.workspaceSortFields.length
         );
         sandbox.assert.callCount(
            spyLogicOnChange,
            mockObj.workspaceSortFields.length
