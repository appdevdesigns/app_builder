...
describe("ABField unit tests", () => {
	...
	describe('ABField test cases', () => {
		...
		it('.migrateCreate: should call and pass valid parameters', () => {
			...
			let stubPost = sandbox.stub(OP.Comm.Service, 'post').callsFake(function () { return Promise.resolve(); });
		...
		it('.migrateUpdate: should call and pass valid parameters', () => {
			...
			let stubPut = sandbox.stub(OP.Comm.Service, 'put').callsFake(function () { return Promise.resolve(); });
		...
		it('.migrateDrop: should call and pass valid parameters', () => {
			...
			let stubDelete = sandbox.stub(OP.Comm.Service, 'delete').callsFake(function () { return Promise.resolve(); });
		...
		it('.isValidData: should add the required field error', () => {
			...
			sandbox.assert.calledWith(stubAddError, target.columnName, '*This is a required field.');
		...
		it('.format: should call .dataValue', () => {
			...
			let stubDataValue = sandbox.stub(target, 'dataValue').callsFake(function () { });
		...
		it('.formComponent.newInstance: should call application.viewNew with valid parameters', () => {
			...
			let stubViewNew = sandbox.stub(sampleApp, 'viewNew').callsFake(function () { });
