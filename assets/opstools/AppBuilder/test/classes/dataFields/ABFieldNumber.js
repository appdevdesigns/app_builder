...
describe("ABFieldNumber unit tests", () => {
	...
	describe('Number field test cases', () => {
		...
		it('should have valid default value', () => {
			...
			let description = L('ab.dataField.number.description', '*A Float or Integer Value');
		...
		it('.columnHeader: should return column config', () => {
			...
			assert.equal('number', columnConfig.editor, 'should be "number" editor');
		...
		it('.isValidData - should not pass when integer value is greater than maximum', () => {
			...
			sandbox.assert.calledOnce(spyAddError);
			sandbox.assert.calledWith(spyAddError, columnName, 'should be less than {max}'.replace('{max}', target.settings.validateMaximum));
		});
