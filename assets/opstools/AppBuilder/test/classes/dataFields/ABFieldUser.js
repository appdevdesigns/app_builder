   ...
   describe("User field test cases", () => {
      ...
      it(".columnHeader: should return valid column config when it is a single select field using richselect and is editable", () => {
         ...
         assert.isDefined(
            ...
            'should not define "template" for a single select user'
         );
         assert.isDefined(
            columnConfig.options,
            "should have default list of users defined."
         );
      ...
      it(".columnHeader: should return valid column config when it is a multiple select field using selectivity", () => {
         ...
         assert.isUndefined(
            columnConfig.editor,
            'should not define "editor" for a single select user'
         );
         assert.isDefined(
            columnConfig.template,
            'should define "template" for a single select user'
         );
         assert.isUndefined(
            columnConfig.sort,
            'should not define "sort" in webix datatable'
      ...
      it(".defaultValue: should set current user", () => {
         ...
         assert.isDefined(
            currentUser[columnName],
            " --> our new column should be inserted into our provided values"
         );
         assert.isArray(currentUser[columnName], " --> should be an array");
         assert.equal(
            currentUser[columnName][0].text,
            OP.User.username(),
            " --> set to what the OP.User is"
         );
      });
   });
});
