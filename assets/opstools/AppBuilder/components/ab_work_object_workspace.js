...
module.exports = class ABWorkObjectWorkspace extends ABComponent {
    ...
    constructor(App, idBase, settings) {
                        ...
                        OP.Dialog.Confirm({
                            ...
                            callback: (result) => {
                                if (result) {
                                    ...
                                    CurrentObject.workspaceViews.removeView(
                                        view
                                    );
        ...
        var CurrentDatacollection = new ABDataCollection(
            {},
            CurrentApplication
        );
        ...
        var _logic = {
            ...
            callbackHeaderEditorMenu: function(action, field, node) {
                switch (action) {
                    case "hide":
                        var newFields = [];
                        var isHidden =
                            CurrentObject.workspaceHiddenFields.filter(
                                (fID) => {
                        ...
                        break;
                    case "filter":
                        _logic.toolbarFilter(
                            $$(ids.buttonFilter).$view,
                            field.id
                        );
                        break;
                    case "sort":
            ...
            callbackViewUpdated: function(view) {
                if (
                    view.id === CurrentObject.workspaceViews.getCurrentView().id
                ) {
                    _logic.switchWorkspaceView(view);
                } else {
            ...
            getBadgeFilters: function() {
                var filterConditions = CurrentObject.currentView()
                    .filterConditions;
            ...
            populateObjectWorkspace: function(object) {
                ...
                PopupSortFieldComponent.setValue(
                ...
                PopupExportObjectComponent.objectLoad(CurrentObject);
                PopupExportObjectComponent.setGridComponent(
                    $$(DataTable.ui.id)
                );
            ...
            refreshViewMenu: function() {
                var currentViewId = CurrentObject.workspaceViews.getCurrentView()
                    .id;
                var submenu = CurrentObject.workspaceViews
                    .list()
                    .map((view) => ({
                        hash: view.type,
                        value: view.name,
