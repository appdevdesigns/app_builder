...
export default class AB_Work_Process_List extends OP.Component {
    ...
    constructor(App) {
        ...
        var ListComponent = new ABCommonList(App, {
            ...
            labels: {
                ...
                confirmDeleteTitle: L(
                    "ab.process.delete.title",
                    "*Delete Process"
                ),
                title: L("ab.process.list.title", "*Processes"),
                searchPlaceholder: L(
                    "ab.process.list.search.placeholder",
                    "*Process name"
                )
            },
            templateListItem:
        ...
        var _logic = (this._logic = {
            addNew: () => {
                this._logic.clickNewProcess(true);
            },
            ...
            clickNewProcess: function(selectNew) {
                // show the new popup
                console.log(
                    ".clickNewProcess(): show popup here!  SelectNew:" +
                        selectNew
                );
                AddForm.show();
            },
            ...
            copy: function(data) {
                ...
                CurrentApplication.processCreate(data.item).then(
                    (newProcess) => {
                        ListComponent.ready();
                        ListComponent.dataLoad(CurrentApplication.processes());
                        ListComponent.select(newProcess.id);
            ...
            exclude: function(process) {
                ListComponent.busy();
                CurrentApplication.processRemove(process).then(() => {
                    ListComponent.dataLoad(CurrentApplication.processes());
