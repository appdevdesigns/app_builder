	...
	constructor(App, attributes) {
		...
		var labels = {
			...
			component: {
				...
				confirmDeleteTitle: L('ab.common.list.delete.title', "*Delete Item"),
				title: L('ab.common.list.title', '*Items'),
				searchPlaceholder: L('ab.common.list.search.placeholder', "*Item name"),
...
// PopupNewProcessComponent.applicationLoad(application);
			...
			remove: () => {
				...
				OP.Dialog.Confirm({
					...
					message: labels.component.confirmDeleteMessage.replace('{0}', selectedItem.label),
					callback: (isOK) => {
		 ...
		 */
		var _templateListItem = attributes.templateListItem || [
