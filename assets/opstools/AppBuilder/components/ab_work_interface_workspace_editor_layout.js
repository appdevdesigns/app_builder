      ...
      var _logic = (this.logic = {
         ...
         viewLoad: function(view) {
            ...
            if (CurrentViewMode == "preview") {
               ...
               if (
                  CurrentView.settings.type == "popup" &&
                  CurrentView.settings.popupWidth &&
                  CurrentView.settings.popupHeight
               ) {
                  $$(ids.editAreaContainer).define({
                     width: parseInt(CurrentView.settings.popupWidth)
                  });
                  $$(ids.editArea).define({
                     height: parseInt(CurrentView.settings.popupHeight)
                  });
                  webix.html.addCss(
                  ...
                  CurrentView.settings.type == "page" &&
                  CurrentView.settings.fixedPageWidth == 1 &&
                  CurrentView.settings.pageWidth
               ) {
                  $$(ids.editAreaContainer).define({
                     width: parseInt(CurrentView.settings.pageWidth)
                  });
         ...
         ready: () => {
            ...
            if (EditArea) {
               ...
               if (EditArea.hideProgress) EditArea.hideProgress();
            }
         }
      });
