   ...
   constructor(App) {
         ...
         rows: [
            {
               ...
               cols: [
                  {
                     view: "label",
                     label: `&nbsp;&nbsp;<span class='fa fa-users'></span> ${L(
                        "ab.user.title",
                        "*Users"
                     )}`,
                     align: "left"
      ...
      let _logic = {
         ...
         show: function() {
            ...
            _logic.loadUserData().then(() => {
               // Set select item of datatable
               if (this._userDC) {
                  let cursor = this._userDC.getCursor();
                  if (cursor) $$(ids.datatable).select(cursor.id);
            ...
            return new Promise((resolve, reject) => {
               ABUser.findAll(cond)
                  ...
                  .then((data) => {
                     ...
                     if (this._userDC) this._userDC.parse(data || []);
         ...
         selectUser: (userId) => {
            if (!this._userDC) return;

            if (userId) this._userDC.setCursor(userId);
            else this._userDC.setCursor(null);
         },
