   ...
   constructor(App) {
      ...
      var _logic = (this.logic = {
         ...
         dataCollectionEdit: function(e, id, trg) {
            var datacollection = CurrentRootPage.dataCollections(function(
               data
            ) {
               return data.id == id;
            })[0];
         ...
         dataCollectionRemove: function(e, id, trg) {
            ...
            if (datacollection) {
               OP.Dialog.Confirm({
                  title: L(
                     "ab.interface.component.datacollection.confirmDeleteTitle",
                     "*Delete Data Source"
                  ),
                  text: L(
                     "ab.interface.component.datacollection.confirmDeleteMessage",
                     "Do you want to delete <b>{0}</b>?"
                  ).replace("{0}", datacollection.label),
                  callback: (result) => {
                     if (result) {
         ...
         listTemplate: function(dataCollection) {
            ...
            if (dataCollection.settings.isQuery) icon = "filter";
