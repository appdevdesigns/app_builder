...
module.exports = class ABViewForm extends ABViewFormCore {
            ...
            return Promise.resolve()
                .then(() => {
                    ...
                    if (oldDcId != null) {
                        ...
                        oldComps.forEach((child) =>
                            destroyTasks.push(() => child.destroy())
                        );
                    }
    ...
    static propertyEditorPopulate(App, ids, view, logic) {
        ...
        $$(ids.labelPosition).setValue(
            view.settings.labelPosition ||
                ABViewFormCore.defaultValues().labelPosition
        );
        $$(ids.labelWidth).setValue(
            view.settings.labelWidth ||
                ABViewFormCore.defaultValues().labelWidth
        );
        $$(ids.height).setValue(
            view.settings.height || ABViewFormCore.defaultValues().height
        );
        $$(ids.clearOnLoad).setValue(
            view.settings.clearOnLoad ||
                ABViewFormCore.defaultValues().clearOnLoad
        );
        $$(ids.clearOnSave).setValue(
            view.settings.clearOnSave ||
                ABViewFormCore.defaultValues().clearOnSave
        );
    ...
    static populateBadgeNumber(ids, view) {
        ...
        if (view.settings.submitRules) {
            $$(ids.buttonSubmitRules).define(
                "badge",
                view.settings.submitRules.length || 0
            );
        ...
        if (view.settings.displayRules) {
            $$(ids.buttonDisplayRules).define(
                "badge",
                view.settings.displayRules.length || 0
            );
        ...
        if (view.settings.recordRules) {
            $$(ids.buttonRecordRules).define(
                "badge",
                view.settings.recordRules.length || 0
            );
    ...
    component(App) {
        ...
        var _ui = {
            // view: "scrollview",
            // height: this.settings.height || ABViewFormCore.defaultValues().height,
            // body: {
            id: ids.component,
            view: "form",
