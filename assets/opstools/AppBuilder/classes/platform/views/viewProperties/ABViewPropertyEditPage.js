...
module.exports = class ABViewPropertyEditPage extends ABViewPropertyAddPage {
		...
		let _logic = {
			...
			setSettings: (view, settings = {}) => {
				...
				if ($selector) {
					...
					$selector.define("value", settings.editForm || this.default.editForm);
					$selector.refresh();
				}
			...
			getSettings: (view) => {
				...
				let selectedItem  = ($selectPopup.config.body.data || []).filter(opt => opt.id == $selector.getValue())[0];
				if (selectedItem) {
	...
	fromSettings(settings = {}) {
		...
		this.settings.editForm = settings.editForm || this.constructor.default.editForm;
