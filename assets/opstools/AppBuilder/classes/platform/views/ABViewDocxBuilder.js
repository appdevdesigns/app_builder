...
module.exports = class ABViewDocxBuilder extends ABViewDocxBuilderCore {
   ...
   static propertyEditorDefaultElements(App, ids, _logic, ObjectDefaults) {
      ...
      return commonUI.concat([
         ...
         {
            view: "fieldset",
            label: L("ab.component.docxBuilder.exportFormat", "*Export:"),
            labelWidth: App.config.labelWidthLarge,
            body: {
               type: "clean",
               padding: 10,
               rows: [
                  {
                     name: "docx",
                     view: "checkbox",
                     label: L("ab.components.docxBuilder.docx", "*DOCX"),
                     labelWidth: App.config.labelWidthLarge
                  },
                  {
                     name: "pdf",
                     view: "checkbox",
                     label: L("ab.components.docxBuilder.pdf", "*PDF"),
                     labelWidth: App.config.labelWidthLarge
                  }
               ]
            }
   ...
   static propertyEditorPopulate(App, ids, view) {
      ...
      $$(ids.docx).setValue(
         view.settings.docx != null ? view.settings.docx : true
      );
      $$(ids.pdf).setValue(view.settings.pdf);
   ...
   static propertyEditorValues(ids, view) {
      ...
      view.settings.docx = $$(ids.docx).getValue();
      view.settings.pdf = $$(ids.pdf).getValue();
   }
   ...
   component(App) {
      ...
      var ids = {
         ...
         buttonPdf: App.unique(idBase + "_buttonPdf"),
      ...
      let menuList = [];
      if (this.settings.docx || this.settings.docx == null) {
         menuList.push({
            id: ids.button,
            view: "button",
            css: "webix_primary",
            ...
            autowidth: autowidth,
            click: () => {
               _logic.getDocxFile().then((blobDocxFile) => {
                  letUserDownload(blobDocxFile, this.filelabel);
               });
            }
         });
      }
      if (this.settings.pdf) {
         menuList.push({
            ...
            label: L("ab.components.downloadPDF", "*Download PDF"),
            autowidth: autowidth,
            click: () => {
               _logic.getPdfFile().then((blobPdfFile) => {
                  letUserDownload(
                     blobPdfFile,
                     this.filelabel
                        .replace(".docx", ".pdf")
      ...
      let _logic = {
         busy: () => {
            ...
            let downloadPdfButton = $$(ids.buttonPdf);
            if (downloadPdfButton) {
               downloadPdfButton.disable();
               downloadPdfButton.define("icon", "fa fa-refresh fa-spin");
               downloadPdfButton.refresh();
            }
         },
         ...
         ready: () => {
            ...
            let downloadPdfButton = $$(ids.buttonPdf);
            if (downloadPdfButton) {
               downloadPdfButton.enable();
               downloadPdfButton.define("icon", "fa fa-file-pdf-o");
               downloadPdfButton.refresh();
            }
         },
                  ...
                  (blobDocxFile) =>
                     new Promise((next, err) => {
                        ...
                        WebViewer(
                           ...
                           // Load DOCX file to the viewer
                           instance.UI.loadDocument(blobDocxFile, {
                           ...
                           docViewer.addEventListener(
                              ...
                              async () => {
                                 ...
                                 const pdfBlob = new Blob([arr], {
                                    type: "application/pdf"
                                 });

                                 // remove PDFtron viewer div
