...
const ABProcessTaskManager = require("./ABProcessTaskManager");
const ABProcessParticipant = require("./ABProcessParticipant");
const ABProcessLane = require("./ABProcessLane");
...
export default class ABApplication extends ABApplicationBase {
                ...
                .then((newObj) => {
                    ...
                    (newObj.fields || []).forEach((f) => {
                        if (f.key == "connectObject") {
                            ...
                            if (linkObject) {
                                refreshTasks.push(
                                    this.objectRefresh(linkObject.id)
                                );
                            }
                        }
                    });
    ...
    viewDestroy(view) {
        ...
        return this.Model.staticData
            .viewDestroy(this.id, resolveUrl)
            .then(() => {
                // TODO : Should update _AllApplications in
            });
    }
    ...
    dataviewLoad() {
        if (this.loadedDataview) return Promise.resolve();

        return new Promise((resolve, reject) => {
            this.Model.staticData
                .dataviewLoad(this.id)
                .catch(reject)
                .then((dataviews) => {
    ...
    dataviewDestroy(dataview) {
        var remaininDataviews = this.dataviews(
            (dView) => dView.id != dataview.id
        );
    ...
    dataviewSave(dataview) {
        var isIncluded =
            this.dataviews((dView) => dView.id == dataview.id).length > 0;
        if (!isIncluded) {
    ...
    dataviewImport(dataviewId) {
        return new Promise((resolve, reject) => {
            this.Model.staticData
                .dataviewImport(this.id, dataviewId)
                .catch(reject)
                .then((newDataview) => {
                    ...
                    // add to list
                    var isIncluded =
                        this.dataviews((q) => q.id == newDataview.id).length >
                        0;
                    if (!isIncluded) {
    ...
    dataviewExclude(dataviewId) {
        return new Promise((resolve, reject) => {
            this.Model.staticData
                .dataviewExclude(this.id, dataviewId)
                .catch(reject)
                .then(() => {
                    // remove query from list
                    this._dataviews = this.dataviews(
                        (dView) => dView.id != dataviewId
                    );
    ...
    processElementNewForModelDefinition(element, process) {
        ...
        switch (element.type) {
            ...
            default:
                var defaultDef = ABProcessTaskManager.definitionForElement(
                    element
                );
                if (defaultDef) {
    ...
    /**
     * @method participantNew(id)
     *
     * return an instance of a new ABProcessParticipant that is tied to this
     * ABApplication->ABProcess.
     *
     * @param {string} id the ABDefinition.id of the participant being created
     * @param {ABProcess} process the process this participant is a part of.
     * @return {ABProcessParticipant}
     */
    // participantNew(id, process) {
    //     var taskDef = ABDefinition.definition(id);
    //     if (taskDef) {
    //         return new ABProcessParticipant(taskDef, process, this);
    //     }
    ...
    /**
     * @method participantNewForModelDefinition(def, process)
     *
     * return an instance of a new ABProcessParticipant that is tied to the
     * given BPMI:Participant definition.
     *
     * @param {BPMI:Participant} element the element definition from our BPMI
     *              modler.
     * @return {ABProcessParticipant}
     */
    // participantNewForModelDefinition(element, process) {
    //     var newParticipant = new ABProcessParticipant({}, process, this);
    //     if (newParticipant) {
    //         newParticipant.fromElement(element);
    //     }
