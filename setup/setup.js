   ...
   if (process.env.NODE_ENV != "production") {
      ...
      if (!fs.existsSync(pathInstall)) {
         ...
         commands.push({
            ...
            log:
               "<green><bold>installing:</bold> webpack dependencies </green>",
            shouldEcho: true
         });

         fs.renameSync(
            path.join(__dirname, ".notsetup"),
            path.join(__dirname, ".setup_install")
         );
      }
      ...
      commands.push({
         command: "npm",
         options: ["run", "build"],
         log: "<green><bold>Building</bold> App Builder  </green>",
         shouldEcho: true
      });
      ...
      AD.spawn
         ...
         .fail(function(err) {
            AD.log.error(
               "<red> appbuilder: webpack testbuild: NPM exited with an error</red>"
            );
            AD.log(err);
         })
         .then(function(code) {
