                  ...
                  (registrationID, cb) => {
                     ...
                     if (!packet.update) {
                        if (!packet.registration.AccountOwner) {
                           ...
                           if (
                              packet.accountHolderRen &&
                              packet.accountHolderFamilyID == "??"
                           ) {
                              resultAccountHolderLinkedToUnknownRen.push(
                                 packet
                              );
                           } else {
...
`;
            ...
            resultUpdatedEntries.forEach((packet) => {
               formatUpdatedEntries[packet.registration.id] =
                  packet.update.accID;
            });

            var stringUpdatedEntries = JSON.stringify(
               formatUpdatedEntries,
               null,
               4
            );
            logContents += `
...
`;
            ...
            resultRegistrationWithoutAccountHolder.forEach((packet) => {
               formatRegistrationWithoutAccountHolder[packet.registration.id] =
                  packet.registration.AccountOwner;
            });

            var stringRegistrationWithoutAccountHolder = JSON.stringify(
               formatRegistrationWithoutAccountHolder,
               null,
               4
            );
            logContents += `
...
`;
            ...
            resultAccountHolderLinkedToUnknownRen.forEach((packet) => {
               formatAccountHolderLinkedToUnknownRen[
                  packet.registration.AccountOwner
               ] = packet.accountHolderRen;
            });

            var stringAccountHolderLinkedToUnknownRen = JSON.stringify(
               formatAccountHolderLinkedToUnknownRen,
               null,
               4
            );
...
`;
            ...
            resultNoMatchingAccountHolder.forEach((packet) => {
               formatNoMatchingAccountHolder[packet.registration.id] =
                  packet.update;
            });

            var stringNoMatchingAccountHolder = JSON.stringify(
               formatNoMatchingAccountHolder,
               null,
               4
            );
            logContents += `
...
`;
            ...
            resultsPossibleNonPOC.forEach((packet) => {
               formatPossibleNonPOC[packet.registration.id] =
                  packet.accountHolderRen;
            });

            var stringPossibleNonPOC = JSON.stringify(
               formatPossibleNonPOC,
               null,
               4
            );
            logContents += `
...
`;
            ...
            fs.writeFile(logFileName, logContents, (err) => {
               ...
               if (err) {
                  console.log(
                     "error saving results to file[" + logFileName + "] :",
                     err.toString()
                  );
               } else {
