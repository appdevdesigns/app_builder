...
var policy = require(path.join(
   ...
   "api",
   "policies",
   "ABModelConvertSailsCondition.js"
));
...
describe("Policy ABModelConvertSailsCondition : ", function() {
   ...
   function looksLikeQBCond(where) {
      assert.property(
         where,
         "glue",
         " returned QB condition should have a .glue at the root element"
      );
   ...
   it("should not modify a QB condition :", function(done) {
      ...
      policy(req, {}, function(err) {
         ...
         assert.deepEqual(
            QBCond,
            req.options._where,
            " : still equals original condition"
         );
         done();
      });
   });
      ...
      policy(req, {}, function(err) {
         ...
         for (var k in expectedResults) {
            assert.deepEqual(
               QBCond[k],
               expectedResults[k],
               "  " + expectedResults[k].rule + " condition"
            );
         }
